...
/**
 * SEO Analytics Dashboard – Single-file React app (Next.js App Router page)
...
const NAV = [
  {
    type: "group",
    label: "Account Setup",
    items: [{ key: "onboarding", label: "Onboarding", icon: "📝" }],
  },
  {
    type: "group",
    label: "SEO",
...
function isVisible(field: any, values: Record<string, any>) {
...
function SidebarNavigation({ current, onSelect }: { current: string; onSelect: (k: string) => void }) {
  return (
    <aside className="w-[28%] xl:w-[30%] min-w-[280px] max-w-[360px] h-screen border-r bg-white/80 backdrop-blur-sm sticky top-0">
          ...
          if (block.type === "group") {
            return (
              <div key={idx} className="space-y-1">
                {block.label.trim() && <div className="px-2 text-[13px] font-medium text-gray-700">{block.label}</div>}
                <ul className="mt-1">
        ...
        <div className="rounded-xl border overflow-hidden">
          ...
          <ul className="p-3 space-y-2 text-sm">
            <li className="p-3 rounded-lg bg-gray-50">
              Security posture for fintech SaaS — <span className="text-gray-600">Draft</span>
            </li>
            <li className="p-3 rounded-lg bg-gray-50">
              Cloud costs explainer — <span className="text-gray-600">In Review</span>
            </li>
            <li className="p-3 rounded-lg bg-gray-50">
              Vendor risk checklist — <span className="text-gray-600">Final</span>
            </li>
          </ul>
        </div>
      </section>
...
function OnboardingView() {
  ...
  const step: any = (onboardingSchema as any).steps[stepIndex];
  const atFirst = stepIndex === 0;
  const atLast = stepIndex === (onboardingSchema as any).steps.length - 1;

  const visibleFields = useMemo(() => {
    return step.fields.filter((f: any) => isVisible(f, values));
  }, [step, values]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 flex items-center gap-2">
        {(onboardingSchema as any).steps.map((s: any, i: number) => (
          <div
            key={s.step}
            className={classNames(
      ...
      <div className="space-y-4">
        {visibleFields.map((f: any) => (
          <Field key={f.id} field={f} value={(values as any)[f.id]} onChange={(v: any) => setValue(f.id, v)} />
        ))}
      </div>
      ...
      <div className="mt-6 flex justify-between">
        ...
        {atLast ? (
          ...
          <button
            ...
            onClick={() => setStepIndex((i) => Math.min((onboardingSchema as any).steps.length - 1, i + 1))}
          >
            Continue
          </button>
        )}
...
function Field({ field, value, onChange }: { field: any; value: any; onChange: (v: any) => void }) {
  ...
  return (
    <div className="space-y-1">
      ...
      {field.type === "text" && <input className={base} value={value || ""} onChange={(e) => onChange((e.target as HTMLInputElement).value)} />}
      {field.type === "url" && (
        <input type="url" className={base} value={value || ""} onChange={(e) => onChange((e.target as HTMLInputElement).value)} placeholder="https://"/>
      )}
      {field.type === "textarea" && (
        <textarea className={classNames(base, "min-h-[96px]")} value={value || ""} onChange={(e) => onChange((e.target as HTMLTextAreaElement).value)} />
      )}
      {field.type === "dropdown" && (
        <select className={base} value={value || ""} onChange={(e) => onChange((e.target as HTMLSelectElement).value)}>
      ...
      {field.type === "file" && (
        <input type="file" className={base} onChange={(e) => onChange((e.target as HTMLInputElement).files?.[0] || null)} />
      )}
...
function MainRouter({ routeKey }: { routeKey: string }) {
  switch (routeKey) {
    case "onboarding":
      return <OnboardingView />;
    case "seo/configuration":
      return <PlaceholderView title="SEO – Configuration" description="Manage crawl settings, content sources, and environment keys." />;
    case "seo/strategy":
      return <PlaceholderView title="SEO – Strategy" description="North-star goals, topic clusters, and prioritization logic." />;
    case "seo/technical-audit":
      return <PlaceholderView title="SEO – Technical Audit" description="Live audit queue, severity scoring, and fix suggestions." />;
    case "seo/off-page-calendar":
      return <PlaceholderView title="SEO – Off Page Calendar" description="Backlink sprints, outreach windows, and content syndication." />;
    case "seo/dashboard":
      return <DashboardView />;
    case "abm/lead-enrichment":
      return <PlaceholderView title="ABM – Lead Enrichment" description="Upload CRM extracts, enrich via Clearbit/DataforSEO, and score." />;
    default:
      return <DashboardView />;
  }
}
